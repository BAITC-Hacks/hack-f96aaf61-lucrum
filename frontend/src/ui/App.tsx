...
export default function App() {
  ...
  async function handleUpload(file?: File) {
    ...
    try {
      ...
      setSelectedItemIds([]);
      setStockError('');
      // These are exact-match API filters. Keeping values from the previous
      // workbook can silently exclude every position in the newly loaded data.
      setWarehouse(warehouses[0] ?? warehouse);
      ...
      setNow(new Date());
      setIsStale(false);
      setUploadMessage(`Файл загружен: строк продаж — ${upload.counts.sales}, предварительных рекомендаций — ${upload.recommendationCount}. Нажмите «Рассчитать», чтобы обновить таблицу.`);
    } catch (cause: unknown) {
      // A rejected upload leaves the previous backend dataset and run intact.
      if (!uploadActivated) setIsStale(false);
          ...
          <div className="luc-workspace">
            <section className="luc-table-card" aria-label="Рекомендации к заказу">
              ...
              <div className="luc-table-scroll" aria-busy={loadingOrders || calculating}>
                ...
                {!loadingOrders && !calculating && !filtered.length && <div className="luc-empty">{lines.length ? 'По выбранным фильтрам рекомендаций нет.' : runId ? 'Расчёт завершён без рекомендаций. Проверьте загруженные продажи и сопоставление поставщиков, а также фильтры склада и категории.' : 'Загрузите книгу или выполните расчёт, чтобы получить рекомендации.'}</div>}
              </div>
