...
import type {
  ...
  OrderRun, StockRecord, UploadResult, Urgency,
} from './types';
...
async function request<T>(path: string, init?: RequestInit, timeoutMs = 20000): Promise<T> {
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new ApiError(`Backend не ответил за ${Math.ceil(timeoutMs / 1000)} секунд. Проверьте соединение и повторите запрос.`);
    }
    throw error;
  } finally {
...
export async function calculate(scope: CalculationRequest): Promise<OrderRun> {
  ...
  const payload = await request<unknown>(`/api/calculate${suffix}`, { method: 'POST' });
  if (!payload || typeof payload !== 'object') throw new ApiError('Эндпоинт расчёта вернул некорректный ответ.');
  const result = payload as Record<string, unknown>;
  if (result.api_version !== 'v1' || typeof result.run_id !== 'string' || !result.run_id.trim()
    || !Array.isArray(result.orders)) {
    throw new ApiError('В ответе расчёта должны быть api_version v1, run_id и массив orders.');
  }
  return parseOrderRun(payload);
}
...
export async function uploadReport(file: File): Promise<UploadResult> {
  ...
  // The backend reparses every workbook in LUCRUM_RAW_DIR after saving this file.
  const payload = await request<unknown>('/api/upload', { method: 'POST', body }, 120000);
  if (!payload || typeof payload !== 'object') throw new ApiError('Эндпоинт загрузки вернул некорректный ответ.');
  const result = payload as Record<string, unknown>;
  const counts = result.counts;
  if (result.api_version !== 'v1' || result.reloaded !== true || typeof result.filename !== 'string'
    || !counts || typeof counts !== 'object' || Array.isArray(counts)) {
    throw new ApiError('В ответе загрузки отсутствует версия API, имя файла, статус перечитывания или счётчики данных.');
  }
  const countRecord = counts as Record<string, unknown>;
  const countKeys = ['sales', 'stock', 'transit', 'suppliers', 'products', 'stockouts'] as const;
  if (countKeys.some((key) => typeof countRecord[key] !== 'number' || !Number.isFinite(countRecord[key]) || countRecord[key] < 0)
    || typeof result.recommendation_count !== 'number' || !Number.isFinite(result.recommendation_count) || result.recommendation_count < 0) {
    throw new ApiError('Ответ загрузки содержит некорректные счётчики данных или рекомендаций.');
  }
  return {
    filename: result.filename,
    recommendationCount: result.recommendation_count,
    counts: Object.fromEntries(countKeys.map((key) => [key, countRecord[key]])) as UploadResult['counts'],
  };
}
